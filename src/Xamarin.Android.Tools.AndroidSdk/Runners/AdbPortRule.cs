...
public record AdbPortSpec (AdbProtocol Protocol, int Port)
{
	...
	public static AdbPortSpec? TryParse (string socketSpec)
	{
		...
		var colonIndex = socketSpec.IndexOf (':');
		if (colonIndex <= 0 || colonIndex >= socketSpec.Length - 1)
			return null;

		var protocolStr = socketSpec.Substring (0, colonIndex);
		var portStr = socketSpec.Substring (colonIndex + 1);
